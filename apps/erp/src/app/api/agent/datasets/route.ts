// `Drop anything` (tabular files): the session user uploads a CSV/XLSX; ERP forwards it to Intelligence under that
// user's delegation. The file is only data for a later proposal; nothing in ERP changes here.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (Number(request.headers.get("content-length") || 0) > MAX + 65536) throw new BffError(413, "Ukuran maksimum 2 MB.");
    const form = await request.formData().catch(() => null);
    const file = form?.get("file");
    if (!(file instanceof File) || !NAME.test(file.name)) throw new BffError(422, "Unggah berkas CSV atau XLSX.");
    if (file.size > MAX) throw new BffError(413, "Ukuran maksimum 2 MB.");
    ...
    return NextResponse.json({ id: body.id, name: body.name, rows: body.profile?.rows ?? 0, columns: body.profile?.columns?.length ?? 0 }, { status: 201, headers });
  } catch (error) {
