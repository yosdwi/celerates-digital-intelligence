...
export default async function MarketingSheetSyncPage() {
  const t = await getTranslations("marketing");
  const [connection] = await db.select().from(sheetConnections).where(eq(sheetConnections.division_key, "marketing"));
  const savedMapping = connection?.column_mapping ? JSON.parse(connection.column_mapping) : {};

  return (
    <div className="min-h-screen">
      <PageHeader icon={RefreshCw} color="bg-brand-500" eyebrow="Marketing" title="Google Sheet Sync">
        <Link href="/marketing" className="text-sm font-medium text-brand-700 hover:underline">&larr; {t("backToLeads")}</Link>
      </PageHeader>

      <main className="px-8 py-8 max-w-2xl mx-auto space-y-6">
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-slate-700">
            {connection ? t("googleSheetConnected") : t("connectGoogleSheet")}
          </h2>
