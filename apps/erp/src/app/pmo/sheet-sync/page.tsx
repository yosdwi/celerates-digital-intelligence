...
export default async function PMOSheetSyncPage() {
  const t = await getTranslations("pmo.sheetSync");
  const [connection] = await db.select().from(sheetConnections).where(eq(sheetConnections.division_key, "pmo_document_tracker"));
  const savedMapping = connection?.column_mapping ? JSON.parse(connection.column_mapping) : {};

  return (
    <div className="min-h-screen">
      <PageHeader
        icon={RefreshCw} color="bg-violet-500" eyebrow="PMO" title={t("pageTitle")}
        subtitle={t("pageSubtitle")}
      >
        <Link href="/pmo" className="text-sm font-medium text-violet-700 hover:underline">&larr; {t("backToDocumentTracker")}</Link>
      </PageHeader>
