...
export default async function PmoInvoicesSheetSyncPage() {
  const t = await getTranslations("pmo.invoices.sheetSync");
  const [connection] = await db.select().from(sheetConnections).where(eq(sheetConnections.division_key, "pmo_invoices"));
  const savedMapping = connection?.column_mapping ? JSON.parse(connection.column_mapping) : {};

  return (
    <div className="min-h-screen">
      <PageHeader
        icon={RefreshCw} color="bg-violet-500" eyebrow="PMO" title="Google Sheet Sync — TM Invoice"
        subtitle={t("pageSubtitle")}
      >
        <Link href="/pmo/invoices" className="text-sm font-medium text-violet-700 hover:underline">&larr; {t("backToInvoices")}</Link>
      </PageHeader>
      ...
      <main className="px-8 py-8 max-w-2xl mx-auto space-y-6">
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <form action={connectSheet} className="space-y-3">
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-slate-700">{t("googleSheetLinkLabel")}</span>
        ...
        {connection && (
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="text-xs text-slate-500 mb-4">{t("mapColumnsSubtitle")}</p>
