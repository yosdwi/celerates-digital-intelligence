...
export function SyncButtons() {
  const t = useTranslations("pmo.invoices.sheetSync");
  ...
  function handlePull() {
    ...
    startTransition(async () => {
      ...
      if (result.ok) setMessage({ type: "ok", text: t("pullSuccessMessage", { imported: result.imported, skipped: result.skipped }) });
  ...
  function handlePush() {
    ...
    startTransition(async () => {
      ...
      if (result.ok) setMessage({ type: "ok", text: t("pushSuccessMessage", { imported: result.imported }) });
      else setMessage({ type: "error", text: result.error });
    });
  }
    ...
    <div className="space-y-3">
      <div className="flex gap-3">
        <button onClick={handlePull} disabled={isPending} className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700 disabled:opacity-50">
          {isPending ? t("processing") : t("pullFromSheet")}
        </button>
        <button onClick={handlePush} disabled={isPending} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50">
          {isPending ? t("processing") : t("pushToSheet")}
        </button>
      </div>
