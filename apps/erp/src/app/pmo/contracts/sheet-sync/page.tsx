...
export default async function ContractsSheetSyncPage() {
  const t = await getTranslations("pmo.contracts.sheetSync");
  const [connection] = await db.select().from(sheetConnections).where(eq(sheetConnections.division_key, "pmo_contracts"));
