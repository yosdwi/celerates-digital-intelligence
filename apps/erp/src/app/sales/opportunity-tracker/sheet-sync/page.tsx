...
export default async function OpportunityTrackerSheetSyncPage() {
  const t = await getTranslations("sales.sheetSync");
  const [connection] = await db.select().from(sheetConnections).where(eq(sheetConnections.division_key, "sales_opportunity_tracker"));
  const savedMapping = connection?.column_mapping ? JSON.parse(connection.column_mapping) : {};

  return (
    <div className="min-h-screen">
      <PageHeader icon={RefreshCw} color="bg-blue-500" eyebrow="Sales" title="Google Sheet Sync — Opportunity Tracker">
        <Link href="/sales/opportunity-tracker" className="text-sm font-medium text-blue-700 hover:underline">&larr; {t("backToOpportunityTracker")}</Link>
      </PageHeader>
