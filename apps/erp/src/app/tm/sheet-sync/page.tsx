...
export default async function TMSheetSyncPage() {
  const t = await getTranslations("tm.sheetSync");
  const [connection] = await db.select().from(sheetConnections).where(eq(sheetConnections.division_key, "tm_talent_assignment"));
  const savedMapping = connection?.column_mapping ? JSON.parse(connection.column_mapping) : {};

  return (
    <div className="min-h-screen">
      <PageHeader
        icon={RefreshCw} color="bg-sky-500" eyebrow="Talent Management" title="Google Sheet Sync — Talent Assignment"
        subtitle={t("subtitle")}
      >
      ...
      <main className="px-8 py-8 max-w-2xl mx-auto space-y-6">
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-slate-700">
            {connection ? t("connected") : t("connectSheet")}
          </h2>
          {connection && (
