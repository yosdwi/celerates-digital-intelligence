...
export default async function TaCandidatesSheetSyncPage() {
  const t = await getTranslations("ta.candidates.sheetSync");
  const [connection] = await db.select().from(sheetConnections).where(eq(sheetConnections.division_key, "ta_candidates"));
  const savedMapping = connection?.column_mapping ? JSON.parse(connection.column_mapping) : {};

  return (
    <div className="min-h-screen">
      <PageHeader icon={RefreshCw} color="bg-amber-500" eyebrow="Talent Acquisition" title="Google Sheet Sync — Candidate">
        <Link href="/ta/candidates" className="text-sm font-medium text-amber-700 hover:underline">&larr; {t("backToCandidate")}</Link>
      </PageHeader>
      ...
      <main className="px-8 py-8 max-w-2xl mx-auto space-y-6">
        ...
        {connection && (
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="text-xs text-slate-500 mb-4">{t("mapColumnsHint")}</p>
