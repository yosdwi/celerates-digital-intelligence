...
export default async function TASheetSyncPage() {
  const t = await getTranslations("ta.sheetSync");
  const [connection] = await db.select().from(sheetConnections).where(eq(sheetConnections.division_key, "ta_requisition"));
  const savedMapping = connection?.column_mapping ? JSON.parse(connection.column_mapping) : {};

  return (
    <div className="min-h-screen">
      <PageHeader icon={RefreshCw} color="bg-amber-500" eyebrow="Talent Acquisition" title="Google Sheet Sync — Requisition">
        <Link href="/ta" className="text-sm font-medium text-amber-700 hover:underline">&larr; {t("backToRequisition")}</Link>
      </PageHeader>

      <main className="px-8 py-8 max-w-2xl mx-auto space-y-6">
        <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-slate-700">
            {connection ? t("connectedTitle") : t("connectTitle")}
          </h2>
          {connection && (
          ...
          <form action={connectSheet} className="space-y-3">
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-slate-700">{t("sheetUrlLabel")}</span>
            ...
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-slate-700">{t("sheetNameLabel")}</span>
        ...
        {connection && (
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-2 text-sm font-semibold text-slate-700">{t("mapColumnsTitle")}</h2>
            <p className="text-xs text-slate-500 mb-4">{t("mapColumnsDesc")}</p>
        ...
        {connection && connection.column_mapping && (
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-2 text-sm font-semibold text-slate-700">{t("syncTitle")}</h2>
            <SyncButtons />
          </section>
        )}
      </main>
